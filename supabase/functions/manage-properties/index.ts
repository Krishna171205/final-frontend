  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      const propertyData = {
        ...
        bhk: Math.max(1, parseInt(body.bhk) || 1),
    ...
    if (req.method === "PUT") {
      ...
      bhk: Math.max(1, parseInt(body.bhk) || existing.bhk),
      baths: Math.max(1, parseInt(body.baths) || existing.baths),
      sqft: Math.max(500, parseInt(body.sqft) || existing.sqft),
